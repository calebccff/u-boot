 ...
 * Basic ARM SMMU-500 driver, assuming a pre-initialised SMMU and no virtual addressing.
 */
 ...
 */
static int find_cb(struct arm_smmu_priv *priv, uint16_t stream_id)
{
	uint32_t val;
	int i;
...
static int arm_smmu_connect(struct udevice *dev)
{
	...
	if (find_cb(priv, mdev->sid) > 0) {
		printf("Found existing context bank for %s, skipping init\n", dev->name);
		return 0;
	}
